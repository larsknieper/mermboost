...
#' @export
mstop.mer_cv <- function(cv_object) {
  return(cv_object$m_opt)
}

#' @export
plot.mer_cv <- function(cv_object, main = NULL) {

  if (is.null(main)) {
    main = "CV Risks"
  }
  avg_risks <- cv_object$avg_risks
  RISKMAT <- cv_object$cv_risks
  ...
  abline(v = cv_object$m_opt, lty = "dashed", col = "gray")
}
...
#' @export
VarCorr.glmermboost <- function(object, iteration = mstop(object)) {
  VarCorr.mermboost(object, iteration)
}
#' @export
VarCorr.mermboost <- function(object, iteration = mstop(object)) {
  object$nuisance()[[iteration]]$Q
}
